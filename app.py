...
def renderizar_bloco(titulo):
    ...
    # Lógica exclusiva para o Jogo D
    if titulo == "JOGO_D":
        ...
        # Inputs de Escanteios
        media_time = st.text_input("Média Casa + Vis", key=f"mt_{titulo}")
        ...
        msg_base = (f"🚨🔥 ALERTA DE CANTOS 🔥🚨\n\n🏆 Campeonato: {camp}\n⚔️ Confronto: {casa} x {vis}\n"
                    ...
                    f"📈 Médias: Time {media_time} | Liga {media_liga}\n🕒 Horário: {hora} (BR)")
        ...
        components.html("""<script src="https://cdn.jsdelivr.net/npm/chart.js"></script><canvas id="cornersChart"></canvas><script>new Chart(document.getElementById('cornersChart'), {type: 'bar', data: {labels: ['O 7.5', 'O 8.5', 'O 9.5'], datasets: [{data: [82,73,61], backgroundColor: ['#22c55e', '#3b82f6', '#f59e0b']}]}});</script>""", height=250)

    else:
        # Lógica original para A, B e C
        camp = st.text_input("Campeonato", key=f"c_{titulo}")
        casa = st.text_input("Casa", key=f"ca_{titulo}")
        vis = st.text_input("Visitante", key=f"v_{titulo}")
