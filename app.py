...
def renderizar_bloco(titulo):
    ...
    if titulo == "JOGO_D":
        ...
        if st.button("📊 ANALISAR", key=f"an_{titulo}"):
            ...
            components.html(f"""<script src="https://cdn.jsdelivr.net/npm/chart.js"></script><canvas id="cChart" height="150"></canvas><script>new Chart(document.getElementById('cChart'), {{type: 'bar', data: {{labels: ['Time', 'Liga'], datasets: [{{data: [{media_time}, {media_liga}], backgroundColor: ['#22c55e', '#3b82f6']}}]}} }});</script>""", height=200)
        ...
        if f"id_{titulo}" in st.session_state:
            def editar(status):
                new = f"{st.session_state[f'msg_{titulo}']}\n\n🏠 Cantos Casa: {int(st.session_state[f'cc_{titulo}'])} | ✈️ Vis: {int(st.session_state[f'cv_{titulo}'])} | 📊 Total: {int(st.session_state[f'cc_{titulo}'] + st.session_state[f'cv_{titulo}'] )}\n\n{status}"
