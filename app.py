...
def renderizar_bloco(titulo):
    with st.container(border=True):
        ...
        if f"probs_{titulo}" in st.session_state:
            ...
            if isinstance(dados, tuple) and len(dados) == 4:
                pc, pv, pe, pg = dados
                ...
                if st.button(f"🚀 ENVIAR {titulo}", key=f"en_{titulo}", type="primary"):
                    if not TOKEN:
                        ...
                        r = requests.post(url, data={"chat_id": CHAT_ID, "text": msg, "parse_mode": "Markdown"}).json()
                        if r.get("ok"):
                            st.success("Enviado!")
                        else:
                            st.error(f"Erro: {r.get('description')}")
            else:
                st.warning("Dados de análise corrompidos. Analise novamente.")
