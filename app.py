...
def renderizar_bloco(titulo):
    ...
    if titulo == "JOGO_D":
        ...
        if st.button("🚀 ENVIAR ALERTA", key=f"en_{titulo}"):
            # Estrutura agrupada conforme solicitado
        ...
        if f"id_{titulo}" in st.session_state:
            def ed(status):
                # Linha para cima (informações) e uma linha para baixo (status)
                info_extras = f"\n\n--------------------\n🏠 Cantos Casa: {int(cc)}\n✈️ Cantos Visitante: {int(cv)}\n📊 Total de Cantos: {int(cc+cv)}"
                status_txt = f"\n\n{status}" if status != "INFO_MOMENTO" else ""
                base_msg = st.session_state[f'msg_{titulo}'].replace(RODAPE, '')
                txt = f"{base_msg}{info_extras}{status_txt}{RODAPE}"
        ...
        # Jogos A, B e C mantidos no padrão original
