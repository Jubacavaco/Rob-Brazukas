...
def jogo_normal(nome):
    ...
    if st.session_state.get(f"analise_{nome}", False):
        ...
        if mid:
            ...
            b1, b2 = st.columns(2)
            if b1.button("⏱️ MOMENTO", key=f"mom_{nome}"): telegram(f"{base}\nPlacar: {ht}\n⚪ Em Andamento", mid)
            if b1.button("✅ HT", key=f"htg_{nome}"): telegram(f"{base}\nPlacar: {ht}\n✅✅✅ GREEN ✅✅✅", mid)
            if b2.button("🏆 FINAL", key=f"fng_{nome}"): telegram(f"{base}\nPlacar HT: {ht}\nPlacar FT: {ft}\n🏆🏆🏆 GREEN FINAL 🏆🏆🏆", mid)
            if b2.button("❌ RED", key=f"red_{nome}"): telegram(f"{base}\nPlacar: {ft}\n❌❌❌ RED ❌❌❌", mid)
